...
import { importTeachers } from "@/app/actions";
...
export default function ImporterPage() {
    ...
    if (status === "loading") return <div className="p-20 text-center animate-pulse text-slate-400 font-bold">Checking authorization...</div>;
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = async (event) => {
            try {
                ...
                if (!Array.isArray(json)) throw new Error("JSON must be an array of teachers");
            ...
            <div className="bg-white rounded-3xl shadow-xl border border-slate-100 overflow-hidden">
                <div className="bg-gradient-to-r from-slate-800 to-slate-900 px-8 py-10 text-white">
                    <div className="flex justify-between items-center">
                        <div>
                            <h1 className="text-3xl font-bold tracking-tight">Bulk Importer</h1>
                            <p className="text-slate-400 mt-2 font-medium">Add multiple teachers via JSON file upload</p>
                        </div>
                        ...
                        <div className="bg-emerald-50 border border-emerald-100 text-emerald-700 p-6 rounded-2xl space-y-2">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="bg-white/50 p-3 rounded-xl">
                                    <p className="text-xs uppercase font-bold text-emerald-600/60">Imported</p>
                                    <p className="text-2xl font-black">{result.imported}</p>
                                </div>
                                <div className="bg-white/50 p-3 rounded-xl">
                                    <p className="text-xs uppercase font-bold text-slate-400">Skipped/Duplicate</p>
                        ...
                        <div className="space-y-4">
                            ...
                            <ul className="space-y-3 text-slate-500 text-sm font-medium">
                                <li className="flex items-start gap-3">
                                    <div className="w-5 h-5 bg-indigo-50 text-indigo-600 rounded-full flex items-center justify-center text-[10px] shrink-0 mt-0.5">1</div>
                                    <span>Prepare a JSON file containing an array of teacher objects.</span>
                                </li>
                                <li className="flex items-start gap-3">
                                    <div className="w-5 h-5 bg-indigo-50 text-indigo-600 rounded-full flex items-center justify-center text-[10px] shrink-0 mt-0.5">2</div>
                                    <span>Each object should have <b>firstName</b>, <b>lastName</b>, and <b>email</b>.</span>
                                </li>
                                <li className="flex items-start gap-3">
                                    <div className="w-5 h-5 bg-indigo-50 text-indigo-600 rounded-full flex items-center justify-center text-[10px] shrink-0 mt-0.5">3</div>
                                    <span>Duplicates based on email will be automatically skipped.</span>
                                </li>
                            </ul>
                            ...
                            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                                <div className="flex items-center gap-2 text-slate-400 mb-2">
                                    <FileJson size={14} />
                                    <span className="text-[10px] font-bold uppercase tracking-wider">Example JSON Format</span>
                                </div>
                        ...
                        <div className="flex flex-col items-center justify-center border-2 border-dashed border-slate-200 rounded-3xl p-8 hover:border-indigo-400 hover:bg-indigo-50/30 transition-all group">
                            <label className="cursor-pointer flex flex-col items-center text-center gap-4">
                                ...
                                <div className="space-y-1">
                                    <span className="text-base font-bold text-slate-900 block">Click to upload JSON</span>
                                    <span className="text-xs text-slate-400 font-medium">Drag and drop also supported</span>
                                </div>
                                <input
                                    type="file"
                                    accept=".json"
